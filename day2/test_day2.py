...
def test_password_constructor():
    ...
    assert password.lowerLimit == 1
    assert password.higherLimit == 3
    assert password.char == 'a'
    assert password.password == 'abcde'

def test_password_valid():
    password = day2.Password('1-3 a: abcde')
    assert password.isValid() == True

def test_password_invalid():
    password = day2.Password('1-3 b: cdefg')
    assert password.isValid() == False

def test_solve1_simple():
    input = """1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc"""
    assert day2.solve1(input) == 2

def test_solve1():
    assert day2.solve1(input) == 477

def test_solve2_simple():
    input = """1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc"""
    assert day2.solve2(input) == 1

def test_solve2():
    assert day2.solve2(input) == 686

input = """6-7 w: wwhmzwtwwk
